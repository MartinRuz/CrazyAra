...
TEST_CASE("Q_std_value_calculation") {
    ...
    REQUIRE(abs(node.get_stdev_value(0) - 3) <= 0.001); //variance after observing value,value,1
...
TEST_CASE("variance_value_calculation_multibatch") {
    float virtualLoss = 1.;
    float value = 4; //0.4;
    bool solveForTerminal = false;
    ChildIdx childIdx = 0;
