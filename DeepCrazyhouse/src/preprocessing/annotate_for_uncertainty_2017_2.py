...
if __name__ == "__main__":
    rtpt = RTPT(name_initials='MR', experiment_name='AnnotateCrazyhouse_17_2', max_iterations=196)
    rtpt.start()
    ...
    for dataset_type in dataset_types:
        ...
        put('\n', engine_init)
        put('setoption name First_Device_ID value 3', engine_init)
        put('\n', engine_init)
        put('setoption name Last_Device_ID value 3', engine_init)
        put('\n', engine_init)
        put('setoption name Model_Directory value /root/CrazyAra/model/CrazyAra/crazyhouse/', engine_init)
        put('\n', engine_init)
        get(engine_init)
        put('setoption name First_Device_ID value 3', engine_search)
        put('\n', engine_search)
        put('setoption name Last_Device_ID value 3', engine_search)
        put('\n', engine_search)
        get(engine_search)
        ...
        if dataset_type == "train":
            for month in range(7, 10):
                zarr_filepaths.extend(glob.glob(
                    'C:/Users/Martin/Documents/Uni/WS22/BA/openinvc/CrazyAra/dataset_crazyhouse/planes/train/**/*2017-0{}*.zip'.format(
                        month)))
            for month in range(10, 13):
                zarr_filepaths.extend(glob.glob(
                    'C:/Users/Martin/Documents/Uni/WS22/BA/openinvc/CrazyAra/dataset_crazyhouse/planes/train/**/*2017-{}*.zip'.format(
                        month)))
