...
if __name__ == "__main__":
    rtpt = RTPT(name_initials='MR', experiment_name='AnnotateCrazyhouse_rest_16', max_iterations=12)
    rtpt.start()
    ...
    for dataset_type in dataset_types:
        ...
        if dataset_type == "train":
            zarr_filepaths.extend(glob.glob(
                '/home/ml-mruzicka/failed/*2016*.zip'))
        ...
        for filepath in zarr_filepaths:
            ...
            put('\n', engine_init)
            put('setoption name First_Device_ID value 6', engine_init)
            put('\n', engine_init)
            put('setoption name Last_Device_ID value 6', engine_init)
            put('\n', engine_init)
            put('setoption name Model_Directory value /root/CrazyAra/model/CrazyAra/crazyhouse/', engine_init)
            put('\n', engine_init)
            get(engine_init)
            put('setoption name First_Device_ID value 6', engine_search)
            put('\n', engine_search)
            put('setoption name Last_Device_ID value 6', engine_search)
            put('\n', engine_search)
            get(engine_search)
            i = 0
            game = []
